...
public class Queen implements Piece {
	...
	// constructor
	public Queen(boolean isWhite, Square startingSquare) {
	...
	private void updateScope(Square startingSquare, int x, int y) { // each call represents the path the Queen can take
		...
		} else if (y == 4 && x > 4) { // if the path is to the right
			updateScope(startingSquare.right, x + 1, y);
		} else if (y == 4 && x < 4) { // if the path is to the left
			updateScope(startingSquare.left, x - 1, y);
		} else if (y < 4 && x == 4) { // if the path is up
			updateScope(startingSquare.up, x, y - 1);
		} else if (y > 4 && x == 4) { // if the path is down
			updateScope(startingSquare.down, x, y + 1);
